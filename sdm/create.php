...
$sql_total = "SELECT COUNT(*) as total FROM sdm";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_FILES["uploaded_file"]["name"])) {
        ...
        if ($uploaded_file["error"] == UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($uploaded_file["tmp_name"], $target_file)) {
                ...
                $sql = "INSERT INTO sdm (direktorat, kompartemen, departemen, jenis_pekerjaan, tipe_borongan, klasifikasi, formasi, aktual, keterangan, file_name) " .
                ...
                if (!$result) {
                    ...
                    // Insert data into riwayat_sdm table with perubahan information
                    $perubahan_info = "Ditambahkan pada " . date("d/m/Y"); // Get current date
                    $sql_riwayat_sdm = "INSERT INTO riwayat_sdm (direktorat, kompartemen, departemen, jenis_pekerjaan, tipe_borongan, klasifikasi, formasi, aktual, keterangan, file_name, perubahan) " .
                    ...
                    $result_riwayat_sdm = $connection->query($sql_riwayat_sdm);

                    if (!$result_riwayat_sdm) {
                        $errorMessage = "Query tidak valid: " . $connection->error;
                    } else {
                        $direktorat = "";
                        $kompartemen = "";
    ...
    if (empty($unit_kerja) || empty($jenis_pekerjaan) || empty($tipe_borongan) || empty($klasifikasi) || empty($formasi) || empty($aktual) || empty($keterangan) || empty($file_name)) {
                    ...
                    <div class="row justify-content-md-center">
                        ...
                        <form method="post" enctype="multipart/form-data">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-6">
                                    <input type="text" class="form-control" name="kompartemen" value="Kompartemen Sumber Daya Manusia" readonly>
                                </div>
                            </div>
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-6">
                                    <select name="departemen" class="form-select">
                                        ...
                                        <option value="Departemen Pembelajaran & Pengembangan SDM ">Departemen Pembelajaran & Pengembangan SDM</option>
                                        <option value="Departemen Remunerasi & Hubungan Industrial">Departemen Remunerasi & Hubungan Industrial</option>
                                        <option value="Kompartemen Sumber Daya Manusia">Kompartemen Sumber Daya Manusia</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row mb-3">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-3 d-grid">
                                    <a class="btn btn-danger" button href=" /tkno/sdm/sdm.php" role="button"><b>Batal</b></a>
                                </div>
                            </div>
                        </form>
                        </div>
        ...
        $(document).ready(function() {
            <?php if (!empty($successMessage)) : ?>
                ...
                $('#successModal').on('hidden.bs.modal', function() {
                    window.location.href = '/tkno/sdm/sdm.php?page=<?php echo $total_pages; ?>#bottom';
                });
            <?php endif; ?>
        });
    </script>
